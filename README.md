...
## 🚀 V2.0.1 最新更新特性 (Release Notes)

* **⚡ 全链路线程并发调节控制**：
  * **书源工坊连通性诊断**：新增 `⚡ 检测线程` 调节选择器，支持 `3`（防限速）、`5`（稳健）、`10`（推荐）、`15`（快速）、`20`（极速）、`30`（狂飙）快捷切换。
  * **选章下载即时并发调节**：在选章下载弹窗（`ChapterModal`）中新增 `⚡ 3. 并发下载线程数` 调节模块，支持 1~16 线程连续滑动与 **防封 2 / 推荐 4 / 极速 8 / 狂飙 16** 一键预设，实时联动底层抓取引擎。
  * **下载管理器设置联动**：为下载管理器配置面板补充了快捷档位按钮组。
* **🧪 智能自适应「检测已勾选书源」**：
  * 在书源工坊中手动勾选书源时，主检测按钮自动智能切换为 **`🧪 检测已勾选 (N)`**，仅对勾选项执行并发检测；未勾选时一键检测全部。
* **🔄 失效书源专属「低速稳健复测」**：
  * 检测出失效书源后，快捷操作栏自动提供 **复测线程选择器**（1 线程极限防封 / 2 线程防限速 / 3 推荐稳健 / 5 快速复测）与 **`🔄 立即复测失效`** 按钮。
  * **智能解封恢复**：复测通过的书源自动从失效列表中移出并取消勾选；真正失效的书源继续保持勾选，方便一键批量清理。
* **📊 实时进度条与运行状态追踪**：
  * 进度条实时动态精准反映当前运行的实际并发线程数，避免跨模式显示滞后。
