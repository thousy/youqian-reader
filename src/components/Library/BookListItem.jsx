...
export function BookListItem({ book, onClick, onDelete }) {
  ...
  return (
    <div className="book-list-item" onClick={onClick} id={`book-list-${book.id}`} style={{ position: 'relative' }}>
      <div className="book-list-cover">
        {book.cover
          ? <img src={book.cover} alt={book.title} />
          : <span style={{ fontSize: '20px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '100%', height: '100%', background: 'var(--bg-button)' }}>📚</span>
        }
      </div>
      ...
      <div className="book-list-actions">
        ...
        {/* 修改分类按钮 */}
        <button 
          className={`reader-toolbar-btn ${showPopover ? 'active' : ''}`}
          title="修改分类"
          onClick={(e) => {
            e.stopPropagation()
      ...
      {/* 列表分类 Popover */}
      {showPopover && (
        <div className="category-popover list-mode" onClick={e => e.stopPropagation()} style={{ right: '80px', top: '40px' }}>
